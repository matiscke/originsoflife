...
\rev{
While we adopt $\Delta T_\mathrm{min} = \SI{1}{\mega\year}$, we find that increasing this timescale to \SI{100}{\mega\year} reduces the fraction of inhabited planets under $H_1$ by less than 10 \%, suggesting that our results are not highly sensitive to $\Delta T_\mathrm{min}$.
}
...
\rev{Due to our ignorance about its true value or even its order of magnitude, we draw $f_\mathrm{life}$ from a log-uniform prior probability distribution.}

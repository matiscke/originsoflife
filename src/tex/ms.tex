...
\begin{figure*}
    ...
    \end{centering}
\end{figure*}
The situation improves with larger samples: for $n = \var{semian_Nsamp2}$, \SI{80}{\percent} \textcolor{red}{of random samples permit conclusive inference (strong true evidence)} under either $H_\mathrm{1}$ or $H_\mathrm{null}$.
...
\textcolor{red}{For instance, one might `cherry-pick' exoplanets for which a biosignature test is performed based on information that is available \textit{a priori} such as \gls{NUV} \citep[generally referred to as `planetary context';][]{catling2018exoplanet} as to maximize the science yield of performing additional observation of exoplanets.}
...
\begin{figure}
    ...
    \begin{centering}
        ...
        \caption{Fraction of inhabited planets for different threshold \gls{NUV} fluxes under the UV hypothesis if the abiogenesis rate is \var{f_life}. Shaded regions denote 90 percent confidence intervals of randomized sample generations.}

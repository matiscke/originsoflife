...
\subsubsection{Semi-analytical analysis}\label{sec:met-semianalytical}
...
\begin{align}
    P(L|\theta,H_3) &=  f_\mathrm{life} \quad \text{if } \theta>F_\mathrm{NUV, min}\\
    P(L|\theta,H_3) &=  0               \quad  \text{otherwise}
\end{align}
...
\begin{figure}
    ...
    \begin{centering}
        \includegraphics[width=\hsize]{figures/inhabited_FGKM.pdf}
...
\begin{figure*}
    ...
    \begin{centering}
        \includegraphics[width=\hsize]{figures/hz_nuv_evo.pdf}
...
\begin{figure*}
    ...
    \begin{centering}
        \includegraphics[width=\hsize]{figures/semian_true_evidence.pdf}
        \caption{Probability to obtain true strong evidence. Left: evidence levels for $H_\mathrm{3}$ and $H_\mathrm{3, null}$ under sample sizes $n = \var{semian_Nsamp1}$ (solid) and $n = \var{semian_Nsamp2}$ (dashed). The vertical lines denote the thresholds for ``strong'' evidence, $BF_{H_i, H_j}$ > 10, and ``extreme'' evidence, $BF_{H_i, H_j}$ > 100. Right: Probability of true strong evidence for $H_\mathrm{3}$ or $H_\mathrm{3, null}$, whichever is smaller, as a function of sample size $n$.}
...
\begin{figure*}
    \script{semian_true_evidence.py}
    \begin{centering}
        \includegraphics[width=\hsize]{figures/semian_evidence-grid.pdf}
...
The same is true for $F_\mathrm{NUV, min}$, where again higher values increase the evidence as the binomial distribution for $H_\mathrm{3}$ gets increasingly skewed and shifted away from the one for $H_\mathrm{3, null}$.
...
Figure~\ref{fig:semian_selectivity} shows how the probability of obtaining true strong evidence for $H_\mathrm{3}$ scales with selectivity $s$, where $s\in]-1,1[$ such that $F_\mathrm{NUV} \sim Beta(1/10^s,1/10^s)$.
...
\begin{figure}
    \script{semian_true_evidence.py}
    \begin{centering}
        \includegraphics[width=\hsize]{figures/semian_selectivity.pdf}
        \caption{Scaling of the probability of obtaining true strong evidence with sample selectivity. Left: Sampling distribution for different selectivity parameters $s$. Right: Resulting \mbox{P(true strong evidence)}. Sampling more extreme values of $F_\mathrm{NUV}$ is more likely to yield strong evidence.}
...
\begin{figure}
    ...
    \begin{centering}
        \includegraphics[width=\hsize]{figures/detections_uv.pdf}
...
Higher values of $F_\mathrm{NUV, min}$ increase the achieved statistical power of the survey, as the dataset under the alternative hypothesis $H_3$ differs more from the null hypothesis with increasing threshold flux.
The same is true for the abiogenesis rate $f_\mathrm{life}$, where higher values increase the evidence for $H_3$.

\begin{figure}
    \script{powergrid.py}
    \begin{centering}
        \includegraphics[width=\hsize]{figures/powergrid.pdf}
        \caption{Statistical power as a function of threshold \gls{NUV} flux and abiogenesis rate. For a given sample size (here: $N=\var{N_nautilus}$), the achieved statistical power of the survey is enhanced for higher values of $F_\mathrm{NUV, min}$ and $f_\mathrm{life}$.}
        \label{fig:powergrid}
    \end{centering}
\end{figure}
...
We show in Sect.~\ref{sec:results_uv} that the constraining power for testing the hypothesis of a minimum past \gls{NUV} flux required for abiogenesis is sensitive on the occurrence of life, the value of this threshold flux, the sample size, and the distribution of sampled past \gls{NUV} fluxes.

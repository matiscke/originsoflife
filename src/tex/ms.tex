...
Under the UV Threshold Hypothesis ($H_1$), the probability of an exoplanet having detectable biosignatures is zero if the \rev{\gls{NUV}} irradiation is less than the threshold, and it is equal to the (unknown) probability of \rev{life emerging and persisting}, $f_\mathrm{life}$, if \gls{NUV} exceeds the threshold for a sufficiently long period of time \rev{$\Delta T_\mathrm{min}$.}
